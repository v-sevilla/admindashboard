...
const Chat = () => {
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      if (chatRef.current && !chatRef.current.contains(event.target) && isVisible.chat) {
        handleClick('chat');
      }
    };
  ...
  return (
    <div ref={chatRef} className={`nav-item absolute right-5 md:right-52 top-16 bg-white dark:bg-[#42464D] p-8 rounded-lg w-96 shadow-lg ${isVisible.chat ? 'visible' : 'hidden'}`}>
